...
The lambda-free syntax is inspired by
[Alberto Luiz's](http://dis.um.es/~alberto/)
...
`lambda-repl` uses normal-order reduction (left outermost) by default.
To use applicative-order reduction (right innermost):
...
expressions either when no further beta-reductions are possible (beta
normal form), or when the resultant term evaluates to itself --
